...
let cron = require('node-cron')
	...
	//Suit PvP\\
	    ...
	    Miku.sendTextWithMentions(m.chat, `@${roof.p2.split`@`[0]} Refuse Suit, Suit Canceled`, m)
...
Please @${m.mentionedJid[0].split`@`[0]} To Type Accept/Reject`
            ...
            status: 'wait',
            waktu: setTimeout(() => {
            if (this.suit[id]) Miku.sendText(m.chat, `_Suit Time Out_`, m)
            delete this.suit[id]
...
case 'charactercheck':
 ...
 case 'dare':
    ...
    if (isBanChat) return reply(mess.bangc)
         ...
         "Telpon mantanlu trus bilang Kangen Kedia",
         "Nyanyi keun lagu terakhir yang elu dengerin",
         "Kirim VN ke mantan/Doi/Cewek lu , Terus bilang 'Halo aku (Sebut namalu) , Mau nelpon sebentar , aku kangen🥺👉🏼👈🏼",
         "Entot Mamat",
